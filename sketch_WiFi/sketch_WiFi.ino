...
  String connectedStr = (connected ? "true" : "false");

  preferences.begin("wifi", true);
  String s = preferences.getString("ssid", "");
  preferences.end();
  
  String savedSsidJson = "\"saved_ssid\":\"\"";
  if (s.length()) {
    String masked = s;
    if (masked.length() > 2) masked = masked.substring(0,2) + String("****");
    savedSsidJson = String("\"saved_ssid\":\"") + masked + String("\"");
  }
  
...
               String("\"ip\":\"") + ip + String("\",\n") + 
               String("\"connected\":") + connectedStr + String(",\n") + 
               savedSsidJson + String("\n") + 
               String("}");

  // Removed serializeJson
...
  }
  
  // Manually construct JSON string for array
  String networksJson = "[\n";
  for (int i=0;i<n;i++) {
    String ssid = WiFi.SSID(i);
    // Escape quotes in SSID if needed (basic version skips this for simplicity)
    
    networksJson += String("{\n") + 
                   String("\"ssid\":\"") + ssid + String("\",\n") + 
                   String("\"rssi\":") + String(WiFi.RSSI(i)) + String(",\n") + 
                   String("\"enc\":") + String(WiFi.encryptionType(i)) + String("\n") + 
                   String("}");
    if (i < n - 1) {
      networksJson += ",\n";
    } else {
...
  networksJson += "]";
  
  String out = String("{\"networks\":") + networksJson + String("}");
